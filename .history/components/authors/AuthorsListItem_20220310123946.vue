      ...
      <v-row :class="{ 'mt-6': index > 0 }" no-gutters>
        ...
        <v-col cols="12" md="8" class="mx-3 py-6">
          ...
          <div class="author-exerpt">
            <nuxt-content :document="highlightDocument(item)" />
          </div>
...
export default {
  ...
  methods: {
    ...
    highlightWord(word = '') {
      return highlight(word, this.$store.state.authors.search || '')
    },
    highlightDocument(document) {
      console.log('document: ', document)
      const search = this.$store.state.authors.search || ''
      // TODO update with base token search variable
      if (search && search.length > 2) {
        const processHighlight = (elements) => {
          elements.forEach((element) => {
            if (element.type === 'text') {
              element.value = highlight(element.value, search)
            } else {
              processHighlight(element.children)
            }
          })
        }
        const highlightedDocument = processHighlight(document.body.children)
