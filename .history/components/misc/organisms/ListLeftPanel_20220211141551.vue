...
export default {
  ...
  computed: {
    search: {
      ...
      async set(v) {
        await this.$store.dispatch(this.type + '/update', { [this.type]: v })
      },
    },
    ...mapState({
      search(state) {
        return state[this.type].search
      },
      items(state) {
        console.log('stateitems: ', state[this.type].items)
        return state[this.type].items
      },
      display: (state) => state[this.type].display,
      views: (state) => state[this.type].views,
      total: (state) => state[this.type].total,
      filters: (state) => state[this.type].filters,
      skip: (state) => state[this.type].skip,
      limit: (state) => state[this.type].limit,
      page: (state) => state[this.type].page,
      sortBy: (state) => state[this.type].sortBy,
      sortDesc: (state) => state[this.type].sortDesc,
      itemsPerPage: (state) => state[this.type].itemsPerPage,
      itemsPerPageArray: (state) => state[this.type].itemsPerPageArray,
      numberOfPages: (state) => state[this.type].numberOfPages,
    }),
    ...mapGetters('list', ['activeFiltersCounter']),
  },
  async mounted() {
    await this.$store.dispatch('list/list', this.type)
  },
  updated() {},
  methods: {
