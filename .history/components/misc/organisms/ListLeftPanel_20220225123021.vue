<template>
  <div>
    ...
    <template v-if="$vuetify.breakpoint.smAndDown">
      <FiltersDialog v-model="showFilters" />
      <v-footer fixed padless>
        <v-btn block large tile color="info" @click="showFilters = true">
          <v-icon left>mdi-filter</v-icon>
          <span>{{ $t('pages.search.filter') }}</span>
          <v-avatar v-if="counter" class="ml-2" color="green" size="24">{{
            counter
          }}</v-avatar>
        </v-btn>
      </v-footer>
