              ...
              >
                <v-col cols="12 ">
                  <div class="mr-4 text-subtitle-1 grey--text">
                    <template
                      v-if="
                        filterCount && !($route.query && $route.query.search)
                      "
                    >
                      {{ $t('searching') }}
                    </template>
                    ...
                    <template v-if="filterCount"
                      >{{ $tc('with-activefilters-filters', [filterCount]) }}
                    </template>
                    <template
                      v-if="
                        filterCount || ($route.query && $route.query.search)
              ...
              >
                ...
                <div v-if="numberOfPages > 1">
                  <v-pagination
                    ...
                    @input="$store.dispatch(type + '/updatePage', $event)"
                  ></v-pagination>
                </div>
                <v-spacer></v-spacer>
...
export default {
  ...
  computed: {
    ...
    filters: {
      get(filter) {
        return this.$store.state[this.type].filters[filter]
      },
      async set(v, filter) {
        await this.$store.dispatch(this.type + '/update', { [filter]: v })
      },
    },
    numberOfPages() {
    ...
    filterCount() {
      return this.$store.state[this.type].filterCount
    },
    items() {
