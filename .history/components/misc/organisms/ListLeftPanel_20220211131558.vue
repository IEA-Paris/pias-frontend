    ...
    >
      ...
      >
        <v-data-iterator
          ...
          :sort-desc="sortDesc"
          hide-default-footer
          @update:page="dispatch('list', type)"
        >
          <template #header>
            <v-container
                ...
                <v-btn
                  ...
                  @click="
                    $router.push({ query: undefined }) &&
                      $store.dispatch('list/updatePage', type, 1)
                  "
                >
              ...
              >
                <v-sheet class="d-flex align-center">
                  ...
                  <v-select
                    ...
                    hide-details
                    @change="$store.dispatch('list/updatePage', type, 1)"
                  ></v-select>
                </v-sheet>
                ...
                <div v-if="numberOfPages > 1">
                  <v-pagination
                    ...
                    @input="$store.dispatch('list/updatePage', type, $event)"
                  ></v-pagination>
                </div>
                <v-spacer></v-spacer>
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      items(state) {
        console.log('stateitems: ', state.list.items)
        return state.list.items
      },
      display: (state) => state.list.display,
      views: (state) => state.list.views,
      total: (state) => state.list.total,
      filters: (state) => state.list.filters,
      skip: (state) => state.list.skip,
      limit: (state) => state.list.limit,
      page: (state) => state.list.page,
      sortBy: (state) => state.list.sortBy,
      sortDesc: (state) => state.list.sortDesc,
      itemsPerPage: (state) => state.list.itemsPerPage,
      itemsPerPageArray: (state) => state.list.itemsPerPageArray,
      numberOfPages: (state) => state.list.numberOfPages,
    }),
