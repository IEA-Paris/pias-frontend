...
export default {
  ...
  computed: {
    currentIcon() {
      return Object.keys(this.items).find((item) => {
          console.log(' this.items[item].value.join(): ',  this.items[item].value.join());
          console.log('  this.$store.state[this.type].sortBy[0] +
            this.$store.state[this.type].sortDesc[0]: ',   this.$store.state[this.type].sortBy[0] +
