<template>
  ...
  >
    <v-row
      v-for="section in Math.min(sections, Math.ceil(payload.items.length / 3))"
      :key="section"
      class="transition-swing"
    ...
    >
      ...
      >
        <component
          ...
          v-if="payload.items[(section - 1) * 3]"
          :item="{ ...payload.items[(section - 1) * 3], highlighted: true }"
          v-bind="$attrs"
          highlighted
      ...
      <v-col cols="12" :md="filter ? 12 : 6" lg="4" xl="6" class="">
        <v-row :no-gutters="!$store.state.scrolled">
          ...
          >
            <component
              :is="type.charAt(0).toUpperCase() + type.slice(1) + 'Item'"
              v-if="payload.items[(section - 1) * 3 + 1]"
              :item="payload.items[(section - 1) * 3 + 1]"
              v-bind="$attrs"
              class="h500"
              :scroll="$store.state.scrolled"
            ></component>
          ...
          >
            <component
              :is="type.charAt(0).toUpperCase() + type.slice(1) + 'Item'"
              v-if="payload.items[(section - 1) * 3 + 2]"
