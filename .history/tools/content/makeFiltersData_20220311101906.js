...
export default async () => {
  ...
  filters.discipline = {
    type: 'Autocomplete',
    items: [
      ...new Set(articles.map((article) => article.discipline).flat()),
    ].filter((item) => item),
  }
  // Thematics filters
  filters.thematic = {
    type: 'Autocomplete',
    items: [
      ...new Set(articles.map((article) => article.thematic).flat()),
    ].filter((item) => item),
  }
  // Type filters
  filters.type = {
    type: 'Autocomplete',
    items: [...new Set(articles.map((article) => article.type).flat())].filter(
      (item) => item
    ),
  }
  // Keyword filters
