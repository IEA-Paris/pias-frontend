...
export default async (
  ...
  Object.keys(query).forEach((filter) => {
    ...
    if (query[filter]?.length && !['page', 'search'].includes(filter)) {
      ...
      if (['tags'].includes(filter)) {
        ...
        if (val.length > 1) {
          pipeline.$or.push([{ category_1: val[0] }, { category_2: val[0] }])
        } else {
          pipeline.$or.push([
            { category_1: { $containsAny: val } },
            { category_2: { $containsAny: val } },
          ])

          pipeline.category_1 = val[0]
        }
      } else {
        pipeline[filter] =
          val.length > 1
  ...
  if (!Object.keys(pipeline.$or)) delete pipeline.$or
