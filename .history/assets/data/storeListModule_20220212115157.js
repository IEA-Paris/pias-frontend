...
import mergeDeep from '~/tools/lib/contentUtilities.js'
...
export const baseMutations = {
  setState(state, values) {
    ...
    if (values.page) {
      if (values.page <= Math.ceil(state.total / state.itemsPerPage)) {
        if (state.page > 1) {
    ...
    mergeDeep(state, values)
    console.log('state2: ', state)
    state.filterCount = Object.keys(state.filters)
      // remove empty values
      .filter(
        (filter) =>
          (typeof state.filters[filter] === 'boolean' &&
            state.filters[filter] !== null &&
            state.filters[filter] !== undefined) ||
          (Array.isArray(state.filters[filter]) &&
            state.filters[filter].length) ||
          (typeof state.filters[filter] === 'object' &&
            Object.keys(state.filters[filter]).length)
      ).length
    console.log(' state.filterCount: ', state.filterCount)
  },
  inc(state) {
    state.inc++
  },
