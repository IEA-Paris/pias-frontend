  ...
  async update({ dispatch, commit, state, getters }, value = {}) {
    ...
    Object.keys(filters).forEach((filter) => {
      ...
      if (
        ...
        } else if((Array.isArray(filters[filter]) && filters[filter].length > 1)){
          pipeline[filter]= {$in:filters[filter] }
        } else {
          pipeline[filter]=Array.isArray(filters[filter])?filters[filter][0]:filters[filter]
        }
        }
      }
    })
