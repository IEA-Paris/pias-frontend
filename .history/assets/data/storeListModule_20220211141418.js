...
export const mutations = {
  update(state, payload) {
    ...
    Object.keys(payload).forEach((key) => {
      if (typeof payload[key] === 'object') state[key] = payload[key]
    })
 ...
 */
  ...
  setState(state, value) {
    ...
    const recursiveSet = (val, localState) => {
      Object.keys(val).forEach((key) => {
        console.log('updating key: ', key)
        if (typeof val[key] === 'object') recursiveSet(val[key], state[key])
        localState[key] = val[key]
      })
    }
    recursiveSet(value, state)
  },
  ...
  setPage(state, value) {
    console.log('value: ', value)
    if (value <= Math.ceil(state.total / state.itemsPerPage)) {
      state.page = value
      if (state.page > 1) {
        this.app.router.push({
...
export const actions = {
  async updatePage({ dispatch, commit, state }, { type, value }) {
    commit('setPage', value)
    await dispatch('list', type)
  },
  updateFilters({ dispatch, commit, state }, value) {
  ...
  async list({ dispatch, commit, state }, type) {
    ...
    pipeline.$or = []
    console.log('filtersRaw[type]: ', filtersRaw[type])
...
export const getters = {
  activeFiltersCounter(state) {
    return Object.values(state.filters).filter((e) => {
      ...
      else if (typeof e === 'object') return Object.keys(e)
      else {
