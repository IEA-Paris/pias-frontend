...
export const mutations = {
  setLogo(state, value) {
    state[type].logo = value
  },
  setScrolled(state) {
    if (process.browser) {
      state[type].scrolled = window.pageYOffset > 0
    }
  },
...
export const actions = {
  ...
  async update({ dispatch, commit, state, getters }, value = {}, type) {
    ...
    const lastPage = Math.ceil(totalItems / state[type].itemsPerPage)

    // use the % (modulus) operator to get a whole remainder
    const lastPageCount = totalItems % state[type].itemsPerPage
    const skipNumber = () => {
      if (+state[type].page === 1) {
        return 0
      }
      if (+state[type].page === lastPage) {
        if (lastPageCount === 0) {
          return totalItems - state[type].itemsPerPage
        }

        return totalItems - lastPageCount
      }
      return (+state[type].page - 1) * state[type].itemsPerPage
    }
    console.log('pipeline: ', pipeline)
    ...
    let items = await this.$content(type, { deep: true })
      .where(pipeline)
      .search(state[type].search)
      .sortBy(state[type].sortBy[0], state[type].sortDesc[0] ? 'desc' : 'asc')
      .skip(skipNumber())
      .limit(state[type].itemsPerPage)
      .fetch()
    // update route
    const query = {
      ...(state[type].search &&
        typeof state[type].search !== 'undefined' && {
          search: state[type].search,
        }),
      ...(state[type].page > 1 && { page: state[type].page.toString() }),
      ...(state[type].sortBy?.length &&
        state[type].sortBy[0] !== getters.defaultSort.value[0] && {
          sortBy: state[type].sortBy[0],
        }),
      ...(state[type].sortDesc?.length &&
        (state[type].sortDesc[0] ? 'desc' : 'asc') !==
          getters.defaultSort.value[1] && {
          sortDesc: !!(
            state[type].sortDesc[0] || getters.defaultSort.value[1] === 'desc'
          ),
        }),
      ...(state[type].view &&
        state[type].view !== getters.defaultView.name && {
    ...
    /*     const isDesc = state[type].sortDesc[0] || getters.defaultSort.value[1]
    const sorter = state[type].sortBy[0] || getters.defaultSort.value[0]
    ...
    if (
      ...
      ['list', 'text'].includes(state[type].view)
    ) {
      ...
      const availableSlots = state[type].itemsPerPage / 3
