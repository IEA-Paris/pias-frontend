...
export const actions = {
  ...
  async list({ dispatch, commit, state }, type) {
    ...
    const filters = Object.keys(state.filters).filter(
      (item) =>
        state.filters[item] === null ||
        (Array.isArray(state.filters[item]) && !state.filters[item].length) ||
        (Object.isObject(state.filters[item]) &&
          !Object.keys(state.filters[item]).length)
    )
    console.log('filters: ', filters)

    Object.keys(filters).forEach((filter) => {
      console.log('filter: ', filter)
      console.log('typeof filter: ', filters[filter])
      const val = JSON.parse(filters[filter])
      if (
        (filters[filter] ||
