...
export const mutations = {
  ...
  loadRouteQuery(state, type, rootState) {
    ...
    let pristine = true
    if (query.search) {
      this.$store.state[type].search = query.search
      pristine = false
    }
    if (query.filters) {
      pristine = false
      ...
      Object.keys(filters).forEach((filter) => {
        Vue.set(this.$store.state[type].filters, filter, filters[filter])
      })
    }
  ...
  setSearch(state, search, type, rootState) {
    this.$store.state[type].search = search
  },
  setItems(state, values, type, rootState) {
    this.$store.state[type].items = values.items
    this.$store.state[type].total = values.total
    this.$store.state[type].numberOfPages = values.numberOfPages
  },
  setItemsPerPage(state, value, type, rootState) {
    this.$store.state[type].itemsPerPage = value
  },
  setPage(state, page, type, rootState) {
    console.log('page: ', page, type, rootState)
    Vue.set(state, 'page', page)
    console.log('this.$store.state[type].apge', this.$store.state[type].page)
  },
  setFilters(state, filters, type, rootState) {
    Vue.set(
      this.$store.state[type].filters,
  ...
  setSort(state, values, type, rootState) {
    ...
    this.$store.state[type].sortBy = [values[0]]
    this.$store.state[type].sortDesc = [values[1] === 'desc']
  },
  setView(state, value, type, rootState) {
    this.$store.state[type].view = value
  },
  setFiltersCount(state, type, rootState) {
    ...
    this.$store.state[type].filtersCount = filtersCount
  },
...
export const getters = {
  filtersCount: (state) => {},
  defaultView: (state, datatype, rootState) => {
    return lists[datatype].views[
      Object.keys(lists[datatype].views).find(
        (item) => lists[datatype].views[item].default === true
      )
    ]
  },

  defaultSort: (state, datatype, rootState) => {
    console.log('type: ', datatype)
    return lists[datatype].sort[
      Object.keys(lists[datatype].sort).find(
        (item) => lists[datatype].sort[item].default === true
      )
    ]
  },
}
