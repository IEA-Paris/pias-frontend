    ...
    const pipeline = {
      ...
      /*  ...state.filters.filter(
        (item) =>
          ...
          (Object.isObject(item) && !Object.keys(item).length)
      ), */
    }
    pipeline.$or = []
    console.log('filtersRaw[type]: ', filtersRaw[type])
    const filters = { ...filtersRaw[type], ...state.filters }
    console.log('filters: ', filters)
    const sorts = sortsRaw[type]
    Object.keys(filters).forEach((filter) => {
      console.log('filter: ', filter)
      console.log('typeof filter: ', filters[filter])
      if (
        (filters[filter] ||
          (Array.isArray(filters[filter]) && filters[filter].length > 0)) &&
