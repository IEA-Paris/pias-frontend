...
export const mutations = {
  ...
  loadRouteQuery(state, type, rootState) {
    ...
    if (query.view) Vue.set(rootState[type], 'view', query.view)
    if (query.page) {
      Vue.set(rootState[type], 'page', query.page)
    } else {
      Vue.set(rootState[type], 'page', 1)
    }
    ...
    if (query.sortBy) Vue.set(rootState[type], 'sortBy', query.sortBy)
    console.log('query.sortDesc: ', query.sortDesc)
    console.log('defaultSort[1]: ', defaultSort[1])
    Vue.set(rootState[type], 'sortDesc', query.sortDesc || defaultSort[1])
  },
