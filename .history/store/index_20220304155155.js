...
export const mutations = {
  ...
  loadRouteQuery(state, type, rootState) {
    ...
    if (query.page) {
      Vue.set(state[type], 'page', { page: query.page, type })
    } else {
      Vue.set(state[type], 'page', { page: 1, type })
    }
    if (query.sortBy) Vue.set(state[type], 'sortBy', query.sortBy)
    console.log('query.sortDesc: ', query.sortDesc)
    if (query.sortDesc) Vue.set(state[type], 'sortDesc', query.sortDesc)
  },
  setSearch(state, values, rootState) {
    state[values.type].search = values.search
  },
  setItems(state, values, rootState) {
    state[values.type].items = values.items
    state[values.type].total = values.total
    state[values.type].numberOfPages = values.numberOfPages
  },
  setItemsPerPage(state, values, rootState) {
    state[values.type].itemsPerPage = values.value
  },
  setPage(state, values, rootState) {
    Vue.set(state[values.type], 'page', values.page)
    console.log('state[type].apge', state[values.type].page)
  },
